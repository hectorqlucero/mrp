...
(defn- capture-deleted-images [f]
  (let [atom (atom [])
        old-var (find-var 'mrp.models.crud/safe-delete-upload!)
        old-val (when old-var (var-get old-var))]
    (try
      (intern 'mrp.models.crud 'safe-delete-upload!
              (fn [img] (swap! atom conj img) true))
      (f atom)
      (finally
        (when old-var
          (intern 'mrp.models.crud 'safe-delete-upload! old-val))))))
...
(deftest build-form-save-strips-zero-size-file-maps-before-regular
  (let [regular-params (atom nil)
    ...
    (with-redefs [mrp.models.crud/process-regular-form
                  (fn [params table & {:keys [conn]}]
                    (reset! regular-params params)
                    true)
                  mrp.engine.config/get-entity-config (fn [& _] nil)
                  mrp.models.crud/process-multi-upload-form
                  (fn [params table entries & {:keys [conn]}]
                    (reset! multi-called true)
                    (reset! upload-entries entries)
                    {:success true})
                  mrp.models.crud/process-upload-form
                  (fn [& _] {:success true})]
