...
(defn- gen-controller
  [handler-name]
  (str "(ns mrp.handlers." handler-name ".controller\n"
       "  (:require\n"
       "   [mrp.handlers." handler-name ".model :as model]\n"
       "   [mrp.handlers." handler-name ".view :as view]\n"
...
(defn- gen-model
  [handler-name]
  (str "(ns mrp.handlers." handler-name ".model\n"
       "  (:require\n"
       "   [mrp.models.crud :refer [Query]]))\n"))

(defn- gen-view
  [handler-name]
  (str "(ns mrp.handlers." handler-name ".view)\n"))
...
(def proutes-path "src/mrp/routes/proutes.clj")

(defn- require-entry
  "Build the require vector string for a handler controller."
  [handler-name]
  (str "[mrp.handlers." handler-name ".controller :as " handler-name "]"))
...
(defn- remove-handler!
  ...
  (let [dir-name (ns-name->path handler-name)
        base-dir (str "src/mrp/handlers/" dir-name)]
